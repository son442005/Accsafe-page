      ...
      <div className="container px-4 md:px-5">
        <div className="flex flex-col lg:flex-row gap-5 md:gap-7">
          <div className="flex-1 space-y-2 md:space-y-3 opacity-0 animate-fade-in-up text-center lg:text-left" style={{ animationDelay: '0.1s', animationFillMode: 'forwards' }}>
            <h3 className="text-2xl md:text-3xl font-bold mb-2 md:mb-3">{t('howto.title')}</h3>
            <p className="text-muted text-base md:text-lg leading-relaxed">{t('howto.desc')}</p>
          </div>
          <div className="flex-1 flex flex-col gap-3 md:gap-3">
            {steps.map((step, index) => (
              ...
              >
                <h4 className="m-0 mb-1.5 md:mb-2 font-semibold text-base md:text-lg">{t(`howto.${step}.title`)}</h4>
                <p className="m-0 text-muted text-sm md:text-base">{t(`howto.${step}.desc`)}</p>
              </div>
            ))}
          </div>
        </div>
